...
function solve(stockLength, kerf, orders) {
  ...
  const layoutPatterns = patterns.map(({ repetition, cuts }) => {
    ...
    return {
      ...
      cutWaste: cutWaste,
      totalWaste: materialWaste + cutWaste,
    };
  });
  ...
  const result = {
    ...
    requiredStocks: layoutPatterns.reduce((acc, curr) => acc + curr.repetition, 0),
    totalParts: orders.reduce((prev, curr) => prev + curr.count, 0),
    totalPartsLength: orders.reduce((prev, curr) => prev + curr.length * curr.count, 0),
    materialWaste: layoutPatterns.reduce(
      (acc, curr) => acc + curr.materialWaste * curr.repetition,
      0
    ),
    cutWaste: layoutPatterns.reduce((acc, curr) => acc + curr.cutWaste * curr.repetition, 0),
  };

  const { materialWaste, cutWaste } = result;
  result.totalWaste = materialWaste + cutWaste;
  const { totalWaste, requiredStocks } = result;
  result.totalWastePercentage = (totalWaste / requiredStocks / stockLength) * 100;
...
export { solve, toFloat, toInt, StringBuffer, round };
